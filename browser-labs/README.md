...
3) STUDENT NOTES
- Что я ожидал более очевидного зависания
- Что произошло (разницы не заметил, ток в первоманс render block ток у css при теге defer, без css и js в блоке)
- Почему это произошло( разницы не заметил, ток в перфоманс render block ток у css при теге defer)
...
3) STUDENT NOTES
- Что я ожидал что left\top лагать перестанет
- Что произошло нечего
- Почему это произошло (что transform  что top left вызывали rendering, через frame rendering transform даже меньше fps, но только painting flashing помогает увиделть разницу)
- Где это важно в реальных проектах
...
3) STUDENT NOTES
- Что я ожидал получить (opaque response, но получал просто 200 и все пусто)
- Что произошло (добавились нужные заголовки)
- Почему это произошло (потому что cors политики нужны правильные заголовки)
- Где это важно в реальных проектах(дда почти везде при общении с беком, почему любой другой клиент не может поставить заголовки(но походу просто url на беке все таки настраиваться  и фронт подстраивается))
